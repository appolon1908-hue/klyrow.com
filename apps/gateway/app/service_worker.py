...
def billing_tick(max_attempts=8):
    with DB() as s:
        unregistered=~select(BillingWorkItem.id).where(BillingWorkItem.billing_event_id==BillingEvent.id).exists()
        for event in s.scalars(select(BillingEvent).where(unregistered).order_by(BillingEvent.created_at).limit(200)).all():
