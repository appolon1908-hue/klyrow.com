...
Open/click tracking uses random opaque tokens stored only as SHA-256 hashes. Tokens are tenant/message/kind scoped, expire, reveal no tenant identifier, and produce a single lifecycle event on repeated access. Tracking stays disabled unless the tenant/stream policy explicitly enables it.
