...
@pytest.mark.asyncio
async def test_event_delivery_denies_plaintext(monkeypatch):
    ...
    with patch("apps.gateway.app.main.httpx.AsyncClient") as client:
        assert await main.emit_middleware("klyrow.test",{"message_id":"test-message"}) is False
        client.assert_not_called()


@pytest.mark.asyncio
async def test_event_delivery_requires_mtls_material(monkeypatch):
    main=importlib.import_module("apps.gateway.app.main")
...
@pytest.mark.asyncio
async def test_event_delivery_uses_verified_client_certificate(monkeypatch,tmp_path):
    main=importlib.import_module("apps.gateway.app.main")
    ...
    with patch("apps.gateway.app.main.ssl.create_default_context",return_value=context) as create_context, \
        ...
        assert await main.emit_middleware("klyrow.test",{"message_id":"test-message"}) is True
