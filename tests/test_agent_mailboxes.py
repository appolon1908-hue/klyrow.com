...
def add_mapping():
    return client.post("/v1/campaign-email-domains",headers=hdr("admin-a@example.com"),json={"campaign_id":"campaign-a","campaign_name":"Campaign A","primary_domain":"codestra.co","sender_domain_verified":True,"inbound_domain_verified":True,"sending_enabled":True,"receiving_enabled":True,"status":"active"})

def test_first_name_normalization():
    assert normalize_first_name("José")=="jose"
    assert normalize_first_name("María José")=="mariajose"
    assert normalize_first_name("Jean-Pierre")=="jean-pierre"
...
def test_provision_replay_conflict_activation_and_sender_isolation():
    ...
    collision=client.post("/v1/agent-mailboxes/provision",headers=admin,json=event(agent="agent-2",event_id="event-2"));assert collision.status_code==409 and collision.json()["detail"]=="EMAIL_ADDRESS_CONFLICT"
