...
def campaign_store(monkeypatch):
    ...
    for model in (
        main.Tenant, main.Campaign, main.Contact, main.Suppression, ScopedSuppression,
        Template, TemplateVersion, SenderIdentity, CampaignVersionSnapshot,
        CampaignAudienceSnapshot, CampaignDispatchRun, CampaignDispatchItem,
        BusinessEventOutbox,
    ):
    ...
    with factory() as session:
        ...
        session.add(SenderIdentity(
            id="sender-a", tenant_id="tenant-a", domain_id="domain-a",
            email="marketing@example.com", stream="MARKETING", status="ACTIVE",
        ))
        session.add(Template(
            id="template-a", tenant_id="tenant-a", slug="launch", name="Launch",
            status="PUBLISHED", current_version=3, locale="en",
        ))
        session.add(TemplateVersion(
            id="template-version-a", tenant_id="tenant-a", template_id="template-a",
            version=3, subject="Frozen subject", html_body="<p>Hello</p>", text_body="Hello",
            variables_json="[]", created_by="owner-a",
        ))
        session.commit()
    yield factory
    engine.dispose()
...
def add_campaign(factory, *, campaign_id="campaign-a", recipients=("one@example.com",), when=None):
    when = when or datetime.now(timezone.utc)-timedelta(seconds=1)
    with factory() as session:
        campaign = main.Campaign(
            id=campaign_id, tenant_id="tenant-a", name="Campaign", status="draft",
            sender_id="sender-a", template_id="template-a", current_version=4,
        )
        session.add(campaign)
        for index, recipient in enumerate(recipients):
            session.add(main.Contact(
                id=f"contact-{campaign_id}-{index}", tenant_id="tenant-a",
                email=recipient, subscribed=True, metadata_json="{}",
            ))
        session.flush()
...
def test_campaign_normal_completion_uses_normal_message_admission(campaign_store, monkeypatch):
    ...
    assert key.startswith("campaign:campaign-a:4:") and correlation.startswith("cmpmsg_")
    with campaign_store() as session:
...
def test_pause_resume_and_cancel_are_fenced_and_durable(campaign_store):
    ...
    with campaign_store() as session:
        ...
        assert session.scalar(select(BusinessEventOutbox)).event_type == "klyrow.campaign.summary"


def test_expired_dispatcher_lease_recovers_without_duplicate_send(campaign_store, monkeypatch):
    add_campaign(campaign_store)
    with campaign_store() as session:
...
def test_suppressed_recipient_is_never_admitted(campaign_store, monkeypatch):
    ...
    with campaign_store() as session:
        session.add(main.Suppression(
            id="suppression-a", tenant_id="tenant-a", email="blocked@example.com", reason="complaint"
        ))
        session.commit()
    calls = []
    fake_success(monkeypatch, calls)
...
def test_transient_admission_failure_retries_with_same_logical_key(campaign_store, monkeypatch):
    add_campaign(campaign_store)
    import apps.gateway.app.provider as provider
    calls = []
    ...
    with campaign_store() as session:
        assert session.scalar(select(CampaignDispatchItem)).state == "DELIVERED"
        assert session.scalar(select(func.count()).select_from(BusinessEventOutbox)) == 1
